$(function(){
	//点击播放
	$(".tvp-play,.tvp-button-play").click(function () {
		if($(".tvp-overlay-poster").length>0){
            $(".tvp-overlay-poster").animate({"opacity":"0"},300,function(){
                $(".tvp-overlay-poster").remove();
            });
            $(".tvp-button-play").addClass("donghua").animate({"opacity":"0"},300,function(){
                $(".tvp-button-play").hide();
            });
		}
