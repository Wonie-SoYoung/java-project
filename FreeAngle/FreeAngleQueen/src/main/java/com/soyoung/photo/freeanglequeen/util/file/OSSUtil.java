...
public class OSSUtil {
    ...
    public static final String BUCKETNAME="free-angle";       //bucket服务器文件名
     ...
     * @throws
     */
    public static void uploadByFilewhen(OSSClient ossClient, File file, String bucketName, String objectName, HttpSession session,String key) {
        try {
            // 带进度条的上传。
            ossClient.putObject(new PutObjectRequest(bucketName, objectName, file).
                    <PutObjectRequest>withProgressListener(new PutObjectProgressListener(session,key)));
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
            if (ossClient != null) {
